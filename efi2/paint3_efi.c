...
#define PLANE_CTL_A         0x70180u
#define PLANE_STRIDE_A      0x70188u
#define PLANE_SURF_A        0x7019Cu
...
#define W           2880
#define H           1800
#define OUR_STRIDE  180            /* 2880*4/64 = 180 */
#define PAGE_SZ     4096u
#define N_PAGES     5063u          /* ceil(2880*1800*4/4096) */
...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    UINT32 orig_ctl    = mmio_r32(mmio, PLANE_CTL_A);
    UINT32 orig_stride = mmio_r32(mmio, PLANE_STRIDE_A);
    UINT32 orig_surf   = mmio_r32(mmio, PLANE_SURF_A);

    /* stage 3: direct GGTT flip on pipe A if active */
    if (!(orig_ctl & (1u << 31))) {
        LOG(L"Pipe A [off] — skipping direct flip\n");
        log_flush(ImageHandle);
        uefi_call_wrapper(SystemTable->BootServices->Stall, 1, 10000000);
        return EFI_SUCCESS;
    }
    ...
    mmio_w32(mmio, PLANE_CTL_A,
             orig_ctl & ~(PLANE_CTL_TILED | PLANE_CTL_RENDER_DECOMP));
    mmio_w32(mmio, PLANE_STRIDE_A, OUR_STRIDE);
    mmio_w32(mmio, PLANE_SURF_A,   OUR_GTT_ADDR);
    ...
    mmio_w32(mmio, PLANE_CTL_A,    orig_ctl);
    mmio_w32(mmio, PLANE_STRIDE_A, orig_stride);
    mmio_w32(mmio, PLANE_SURF_A,   orig_surf);
    mmio_r32(mmio, PLANE_SURF_A);
